...
int main() {
	...
	my_gizmo.set_target(window).set_color(sf::Color::Blue); // font can only be undefined if you never write text with these properties

	// If SFTK_GIZMO_GLOBAL_PROPERTIES is defined, you can use the global variable `sftk::gizmo` instead
	sftk::gizmo.set_target(window).set_color(sf::Color::Red).set_font(font).set_character_size(40);
	...
	while (window.isOpen()) {
		...
		my_gizmo.draw_arc({ 600, 500 }, 40, 320, 260)
		...
		sftk::draw_circle({ 400, 400 }, 50).draw_arc({ 600, 400 }, 50, 240, 30); // ... which can also be chained
