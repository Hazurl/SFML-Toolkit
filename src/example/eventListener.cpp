...
int main() {
	...
	sftk::Listener closer(
		...
		[](sf::Window&, sf::Event::KeyEvent const&, sftk::pressed_tag_t) {
			// only executed if the the key is not Escape, since the above function will stop the propagation if that's
			// the case
	...
	std::initializer_list<sftk::EventListener*> listeners = { &key_listener, // higher priority at the top
															  &closer };
