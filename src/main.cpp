...
enum class MinSizeStrategy {
    Clamp, Proportional
};

std::array<sf::Vertex, 54> make_gui_rect_vertices(std::array<float, 4> const& xs, std::array<float, 4> const& ys, std::array<float, 4> const& us, std::array<float, 4> const& vs) {
...
std::pair<std::array<float, 4>, std::array<float, 4>> get_ux_or_vy(float const left, float const right, float const texture_size, float const size, MinSizeStrategy const strategy) {
    if (left + right <= size) {
        return {
...
std::array<sf::Vertex, 54> make_gui_rect(sf::Texture const& texture, Padding const& padding, sf::Vector2f const& size, MinSizeStrategy const strategy) {
...
class GUISprite : public sf::Transformable, public sf::Drawable {
private:
