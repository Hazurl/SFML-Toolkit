...
sf::Color to_rgb(sf::Uint8 hue, sf::Uint8 saturation, sf::Uint8 luminosity, sf::Uint8 alpha) noexcept {
	...
	return sf::Color(
		static_cast<sf::Uint8>((r + m) * 255.f), static_cast<sf::Uint8>((g + m) * 255.f), static_cast<sf::Uint8>((b + m) * 255.f), alpha);
}

std::ostream& operator<<(std::ostream& os, HSLColor c) {
	return os << "HSLColor {" << static_cast<unsigned>(c.h) << ", " << static_cast<unsigned>(c.s) << ", " << static_cast<unsigned>(c.l) << ", "
